...
void generatemesh()
{
    ...
    for (i=0; i<=n; i++)
    {
        for (j=0; j<=m; j++)
        {
            ...
            //记录物理坐标
            point[i][j].x = -1.0 + hx * i;
            point[i][j].y = -1.0 + hy * j;
            
            point[i][j].x0 = -1.0 + hx * i;
            point[i][j].y0 = -1.0 + hy * j;
        }
    }
...
void initial()
{
    ...
    for (i=0; i<n; i++)
    {
        for (j=0; j<m; j++)
        {
            ...
            o[i][j].rho0 = rho;
            o[i][j].rholast = rho;
            o[i][j].taulast[0] = ini_p(xt,yt) / ( rho * (gamma - 1))
                                 + 0.5 * (ini_ux(xt,yt) * ini_ux(xt,yt) + ini_uy(xt,yt) * ini_uy(xt,yt));
